          ...
          <div key={bus.id} className="bg-white rounded-[var(--radius)] border border-border shadow-[var(--shadow-card)] overflow-hidden">
            <div className="flex items-center justify-between px-5 py-3 bg-[#1b4332] text-white">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => sharePDFOne(bus)} disabled={pdfLoadingId === bus.id}
                  className="text-xs px-3 py-1 border border-white/40 rounded hover:bg-white/10 transition-colors flex items-center gap-1 disabled:opacity-50">
                  {pdfLoadingId === bus.id ? (
                  ...
                  {pdfLoadingId === bus.id ? "מכין..." : "שתף PDF"}
                </button>
